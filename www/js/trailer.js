...
const Trailer = {
  ...
  // видео под трек magic-energy (звучит целиком, без склеек): смена каждой сцены — на акценте трека
  // (дроп 60,46 — на переход в коконы, мощный вход 31,70 — на карту, последний удар 137,32 — на логотип)
  VIDEO_DUR: { city: 5680, crack: 5840, koschey: 6940, swarm: 5620, hide: 7620, map: 7160, springs: 7020, catch: 7780, evolve: 6800, cocoon: 7400,
    dex: 7340, clans: 7360, duel: 6420, raid: 8320, auction: 10360, weather: 8040, wardrobe: 6880, league: 7500, mentor: 7240 },
  ...
  play(o = {}) {
    return new Promise(done => {
      ...
      const run = () => {
        ...
        this.SCENES.slice(i0, o.only ? i0 + 1 : undefined).map(sc => o.video && this.VIDEO_DUR[sc[1]] ? [this.VIDEO_DUR[sc[1]], sc[1], sc[2]] : sc).forEach(([dur, cls, text], k) => {
